...
export function TableToolbar({
  ...
  table: ReturnType<typeof useUnassignedSubjectTable>;
  tableState: TableState;
  setTableState: Dispatch<SetStateAction<TableState>>;
  entityData: UnassignedSubjectRow[];
  onRefresh: () => void;
}) {
  const hasFilters =
      ...
      <DataTableToolbarGroup className="transition-all duration-300 ease-in-out">
        ...
        >
          ...
          <SelectContent side="bottom">
            {(() => {
              const uniqueTypes = Array.from(
                new Set(entityData.map((item) => item.day).filter(Boolean))
              );
        ...
        >
          ...
          <SelectContent side="bottom">
            {(() => {
              const uniqueTypes = Array.from(
                new Set(
                  entityData.map((item) => item.subject?.type).filter(Boolean)
                )
              );
