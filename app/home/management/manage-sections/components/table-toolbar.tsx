import { SectionRow, TableState } from "../section-manager";
import {
  DataTableToolbar,
...
export function TableToolbar({
  ...
  table: ReturnType<typeof useSectionTable>;
  tableState: TableState;
  setTableState: Dispatch<SetStateAction<TableState>>;
  entityData: SectionRow[];
  ...
  return (
    <DataTableToolbar>
      <DataTableToolbarGroup className="transition-all duration-300 ease-in-out">
          ...
          <SelectContent side="bottom">
            {(() => {
              const uniqueTypes = Array.from(
                new Set(
                  entityData
                    .map((item) => item.course?.academicLevel?.name)
                    .filter(Boolean)
                )
              );
        ...
        <Select
          value={
            (tableState.columnFilters.find((f) => f.id === "course")
              ?.value as string) ?? ""
          }
          onValueChange={(value) =>
            setTableState((prev) => {
        ...
        >
          ...
          <SelectContent side="bottom">
            {(() => {
              const uniqueTypes = Array.from(
                new Set(
                  entityData.map((item) => item.course?.name).filter(Boolean)
                )
              );
        ...
        <Select
          value={
            (tableState.columnFilters.find((f) => f.id === "year")
              ?.value as string) ?? ""
          }
          onValueChange={(value) =>
            setTableState((prev) => {
        ...
        >
          ...
          <SelectContent side="bottom">
            {(() => {
              const uniqueTypes = Array.from(
                new Set(entityData.map((item) => item.year).filter(Boolean))
              );
