import { InstructorRow, TableState } from "../archived-instructor-manager";
import {
  DataTableToolbar,
  ...
  return (
    <DataTableToolbar>
      <DataTableToolbarGroup className="transition-all duration-300 ease-in-out">
        ...
        <Select
          value={
            (tableState.columnFilters.find((f) => f.id === "status")
              ?.value as string) ?? ""
          }
          onValueChange={(value) =>
            setTableState((prev) => {
        ...
        >
          <SelectTrigger>
            <Filter className="text-muted-foreground/80" />
            <SelectValue placeholder="Academic Qualification" />
          </SelectTrigger>
          <SelectContent side="bottom">
            {(() => {
              const uniqueTypes = Array.from(
                new Set(
                  entityData
                    .map((item) => item.academicQualification?.name)
                    .filter(Boolean)
                )
              );
      ...
      <DataTableToolbarGroup>
        ...
        {selectedRowsCount > 1 && (
          <AlertDialog
            open={entityManagement.isRestoreSelectedDialogOpen}
            onOpenChange={entityManagement.setIsRestoreSelectedDialogOpen}
          >
            <AlertDialogTrigger asChild>
              <Button variant="outline">
                <ArchiveRestoreIcon />
